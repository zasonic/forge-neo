...
export function ParamsPanel(): ReactElement {
  ...
  const randomSeed = useTxt2ImgStore((s) => s.randomSeed);
  const recycleSeed = useTxt2ImgStore((s) => s.recycleSeed);
  const showPreview = useTxt2ImgStore((s) => s.showPreview);
  ...
  return (
    <div className="p-4 space-y-4 text-sm">
      ...
      <details className="rounded border border-border bg-bg-panel/40">
        <summary className="px-3 py-2 cursor-pointer text-white/80 select-none flex items-center gap-2">
          <input
            type="checkbox"
            checked={form.enable_hr}
            onChange={(e) => setField('enable_hr', e.target.checked)}
          ...
          <div className="p-3 grid grid-cols-2 gap-3 border-t border-border">
            <Field label="Upscale by">
              <input
                ...
                step={0.05}
                value={form.hr_scale}
                onChange={(e) => setField('hr_scale', Number(e.target.value))}
                className="w-full px-2 py-1.5 rounded bg-bg-panel border border-border"
              />
            </Field>
            ...
            <Field label="Denoising strength">
              <input
                ...
                max={1}
                step={0.05}
                value={form.denoising_strength ?? 0.7}
                onChange={(e) =>
                  setField('denoising_strength', Number(e.target.value))
                }
                className="w-full px-2 py-1.5 rounded bg-bg-panel border border-border"
              />
            </Field>
