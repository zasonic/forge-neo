...
import { InitImagePanel } from './InitImagePanel.js';
...
export function Img2ImgPage(): ReactElement {
  const status = useAppStore((s) => s.status);
  if (status.kind !== 'ready') {
    return (
      <div className="h-full flex items-center justify-center text-white/50 text-sm p-6 text-center">
        Backend is {status.kind}. Start it from the status bar to use Img2Img.
      </div>
    );
  }
  return (
