import type { ReactElement } from 'react';
import { useImg2ImgStore } from '../../lib/img2imgStore.js';
import { useImg2Img, useInterrupt } from '../../hooks/useSdApi.js';

export function PromptPanel(): ReactElement {
  const form = useImg2ImgStore((s) => s.form);
  const setField = useImg2ImgStore((s) => s.setField);
  const setResult = useImg2ImgStore((s) => s.setResult);
  const i2i = useImg2Img();
  const interrupt = useInterrupt();

  const generate = (): void => {
    i2i.mutate(form, {
      onSuccess: (data) => setResult(data),
    });
  };

  const cancel = (): void => {
    interrupt.mutate();
    i2i.abort();
  };

  const pending = i2i.isPending;
  const hasInit = form.init_images.length > 0;
  const canGenerate = !pending && hasInit && form.prompt.trim().length > 0;

  return (
    <div className="p-4 space-y-3">
      <textarea
        value={form.prompt}
        onChange={(e) => setField('prompt', e.target.value)}
        placeholder="Prompt"
        rows={4}
        className="w-full px-3 py-2 rounded bg-bg-panel border border-border text-sm font-mono resize-y"
      />
      <textarea
        value={form.negative_prompt}
        onChange={(e) => setField('negative_prompt', e.target.value)}
        placeholder="Negative prompt"
        rows={2}
        className="w-full px-3 py-2 rounded bg-bg-panel border border-border text-sm font-mono resize-y"
      />
      <div className="flex items-center gap-2">
        <button
          onClick={generate}
          disabled={!canGenerate}
          className="px-4 py-2 rounded bg-accent text-accent-fg disabled:opacity-50 text-sm"
          title={!hasInit ? 'Add an init image first' : undefined}
        >
          {pending ? 'Generating…' : 'Generate'}
        </button>
        {pending && (
          <button
            onClick={cancel}
            className="px-3 py-2 rounded bg-white/10 hover:bg-white/20 text-sm"
          >
            Interrupt
          </button>
        )}
        {i2i.isError && (
          <div
            className="text-xs text-red-300 flex-1 truncate"
            title={i2i.error.message}
          >
            {i2i.error.message}
          </div>
        )}
      </div>
    </div>
  );
}
